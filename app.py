...
if 'equity_history' not in st.session_state:
    # Initialize with a dummy 0 so the chart shows up immediately
...
st.markdown(f"""
    ...
    /* STRAIGHT LINE CENTERED BUTTONS */
    ...
    .glass-card {{
        ...
        border: 1px solid #1c1c1c; text-align: center; margin-bottom: 10px;
    }}
...
if not st.session_state.view_history:
    ...
    st.markdown('<div style="color:#444; font-size:10px; font-weight:800; margin-top:10px; text-transform:uppercase;">Live Execution Log</div>', unsafe_allow_html=True)
    ...
    if df_t.empty:
        st.markdown('<div style="color:#222; text-align:center; padding:20px; font-weight:700;">CONNECTED TO MAINNET...</div>', unsafe_allow_html=True)
    else:
